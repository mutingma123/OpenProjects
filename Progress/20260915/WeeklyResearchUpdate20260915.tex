...
\begin{center}
  ...
  \parbox{0.80\textwidth}{\small\centering
    \fin This report covers one project, the revision of the bi-objective connected and automated vehicle intersection manuscript, since the September 8, 2026 meeting. \fine
...
\fin The September 8, 2026 comments name the experiments as the remaining concern and retarget the paper from Transportation Research Part C to Transportation Research Record or the Journal of Intelligent Transportation Systems \cite{gong2026}. \fine
\fin The six requested experiments are stochastic timing errors on every trajectory, at least thirty seeds per configuration, synchronized, random, and platooned arrivals, one, two, and three vehicles per lane with rear-end exposure admitted, turning splits of 25, 50, and 75 percent, and stronger baselines. \fine
\fin The deliverable is therefore the evidence, not the exposition, and the baseline set carries six controller classes with the published speed-maximization method mapped onto the delay-only class \cite{ma2023speed}. \fine
...
\begin{tightitems}
  \item \fin Deliver the six experiments of the September 8 note as schedules that are certified optimal, independently validated, and reproducible from saved artifacts. \fine
\end{tightitems}
...
\begin{tightitems}
  \item \fin \statusdone Expanded the six requested experiments into seven cases by thirty seeds by twenty-one rows, which is 4,410 report rows, then rebuilt the study as eleven component contracts and a matching \texttt{biobj} runtime package, retiring the legacy BiCAVO codebase after its audit against that plan. \fine
...
\bibitem{gong2026}
\fin J. Gong, ``Re: TRC manuscript status and help needed for submission,'' comments on the manuscript experiments and submission venue, September 8, 2026. \fine
Held locally as \texttt{Comments/Gong/20260908/TRRComments.pdf} in the manuscript repository.
